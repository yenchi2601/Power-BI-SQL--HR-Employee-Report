...
DuplicateNames AS (
    ...
    FROM EmployeeData
    GROUP BY [Full Name]
    HAVING COUNT(DISTINCT [Staff Number]) > 1
)
SELECT *
FROM EmployeeData ed
WHERE ed.[Full Name] IN (SELECT [Full Name] FROM DuplicateNames)
ORDER BY [Surname], [Firstname], [Staff Number];
